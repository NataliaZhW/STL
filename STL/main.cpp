...
void main()
{
	...
	//vec.resize(15);	//задает фактический размер ¬ектора.
	//vec.shrink_to_fit();//урезает вместительность до фактического размера.
	vec.reserve(120);	//позвол€ет зарезервировать нужный объем пам€ти.
	//Ётот метод выполн€етс€ только в том случае, если ему передать значение,
